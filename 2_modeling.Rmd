...
```{r, read in data}
all_kids <- read.csv(here::here('./ha_matched_kids.csv')) %>% # contains the N=38 kids who are hearing age-matched
...
all_kids2 <- all_kids %>%
  distinct(ChildStudyID, .keep_all = TRUE) %>%
  group_by(ResearchID) %>% 
  # classify which visit it was (first, second, or third)
  mutate(num_visit = str_sub(Study, -1)) %>% 
  select(num_visit, ChildStudyID, ResearchID) %>%
...
#write.csv(all_kids2, '/Users/megcychosz/Google Drive/CI_mispron/CI_mispron/project_results/all_kids2.csv') # write out dataframe for replication script
```
...
two_tp <- all_kids2 %>%
  ...
  distinct(ChildStudyID, .keep_all = TRUE) %>%
  add_count() %>%
  distinct(ResearchID, .keep_all = TRUE) %>%
  filter(n == 2) 
...
if (file.exists("../BU2017_results/data/scores.csv")) {
  demo_data <- read.csv("../BU2017_results/data/scores.csv") %>%
    select(ResearchID,AAE,LateTalker) %>%
...
stat_sum_ha <- all_kids2 %>% 
  ...
  group_by(Group) %>% 
  summarize(
    mean_hearing_age = mean(hearing_age),
  ...
  mutate_at(vars(-Group), function(x) round(x, 2)) %>%
  distinct(Group, .keep_all = TRUE) %>%
  mutate(
    `Chronological Age: months` = paste0(mean_age," (",sd_age,") ",min_age,"--",max_age),
    `Hearing Age: months` = paste0(mean_hearing_age," (",sd_hearing_age,") ",min_hage,"--",max_hage),
    `Maternal Education Level` = paste0(mean_mated," (",sd_mated,") "),
    `EVT-2 GSV Scores` = paste0(mean_evt_gsv," (",sd_evt_gsv,") ",min_evt_gsv,"--",max_evt_gsv),
    `EVT-2 Stan. Scores` = paste0(mean_evt_stan," (",sd_evt_stan,") ",min_evt_stan,"--",max_evt_stan)
  ) %>%
  select(tail(names(.),5))
...
```{r, contending with autocorrelation, fig.show='hide',cache=T}
...
# plot the autocorrelation and take a look at 
# correlation between timepoints - yikes!
m_acf_all <- acf_resid(m1_all) 

# first we calculate rho from the original model 
r1_all <- start_value_rho(m1_all) 
...
m_acf_rmv_all <- acf_resid(m_AR1_all) 
...
# Now, I have to see if I am justified in adding a group-level 
# effect to the condition-only model. 
...
condition_base <- bam(
  elog ~ Condition + 
    ...
    s(Time, ChildStudyID, bs = "fs", m = 1) +
    s(Time, TargetWord, bs = "fs", m = 1) +
      # we purposely don't include a smooth of Time by Child by Condition because we're comparing 
      # this nested model to one with GroupCondition below
    s(Time, num_visit, bs = "fs", m = 1), 
    data = all_kids2,           
    rho = r1_all,
    method = "ML",
...
# I am justified in continuing the modeling 

# interact model has much lower AIC
AIC(condition_base, interact_model) 

interact_model_freml <- all_kids2 %>%
...
dir.create(here::here("hearingage_matched_models"), showWarnings = FALSE)
saveRDS(condition_base, here::here("hearingage_matched_models/condition-base-model.Rdata"))
saveRDS(interact_model, here::here("hearingage_matched_models/interact-model.Rdata"))
saveRDS(interact_model_freml, here::here("hearingage_matched_models/interact-model-freml.Rdata"))
saveRDS(interact_model_summary, here::here("hearingage_matched_models/interact-model-summary.Rdata"))
```

```{r, create ordered factors}
# Next, I'll evaluate if the difference between real and mispronounced words is
# significant for the two hearing groups **separately**. First, I have to create
# two ordered factors for a 2x2 relationship of word and condition.

# we have to create *two* reference levels: one for children with CIs and one
# for children with TH Unlike a binary difference smooth, an ordered model
# doesn't lump intercepts and smooths together and I'd like to be able to tease
# them apart in these models
# contrast coding
all_kids2$THReal0 <- as.ordered(all_kids2$Group=='NormalHearing' & all_kids2$Condition=='real')
contrasts(all_kids2$THReal0) <- "contr.treatment" 
...
groupcond_model <- bam(
  elog ~      Group + # ordered variable
                  ...
                  s(Time, ChildStudyID, bs = "fs", m = 1) + 
                  s(Time, TargetWord, bs = "fs", m = 1) +
      # we purposely don't include a smooth of Time by Child by Group because that's not a hierarchical variable
                  s(Time, num_visit, bs = "fs", m = 1), 
                  data=all_kids2,
                  rho=r1_all,
                  method="fREML",
...
saveRDS(groupcond_model, here::here("hearingage_matched_models/group-cond-model.Rdata"))
saveRDS(groupcond_model_summary, here::here("hearingage_matched_models/group-cond-model-summary.Rdata"))
```

```{r, model criticism for 2x2 model, eval=FALSE}
gam.check(groupcond_model) # residuals look fine, k' and k-index values looks fine
```
...
To evaluate how access to fine, phonetic detail may affect mispronunciation sensitivity, a series of GAMMs were fit comparing children with CIs and their hearing age- and vocabulary size-matched peers. **Condition** (Correct Pronunciation vs. Mispronunciation) was contrast-coded to facilitate model interpretation and the 2x2 relationship of **Group** (Children with CIs vs. NH) and **Condition** was modeled using ordered factors. A model with parametric and smooth terms for **Group** and **Condition** improved upon a **Condition**-only model, suggesting that children with CIs and TH responded differently to correct- versus mis-pronunciations. 
...
interact_model_freml <- readRDS("../hearingage_matched_models/interact-model-freml.Rdata")

get_gam_predictions(interact_model_freml, # get the dataframe with fit and s.e. of fit
                      Time,
...
ci_diff <- get_smooths_difference(interact_model_freml, 
                       Time, 
...
th_diff <- get_smooths_difference(interact_model_freml, 
                       Time, 
...
real_diff <- get_smooths_difference(interact_model_freml, 
                       Time, 
  ...
  scale_color_manual(values=c('darkslateblue', 'deeppink3')) +
  scale_fill_manual(values=c('darkblue', 'deeppink3')) +
  ylab("Est. Difference between hearing groups") +
  xlab("Time since word onset (ms)") +
  theme(legend.position = "none",
        axis.text.y = element_text(face="bold", size=9),
        axis.text.x = element_text(face="bold", size=9),
...
MP_diff <- get_smooths_difference(interact_model_freml, 
                       Time, 
...
prepre_looks <- read.csv("../BU2017_results/data/model.csv.gz") %>% 
  mutate(
    Cond_Lab = Condition %>% 
      factor(c("real", "MP", "nonsense"),
             c("Real word", "Mispronunciation", "Nonword")))

looks_a <- read.csv("../BU2017_results/data/scores.csv") %>%
...
hearing_age <- read.csv('../participant_info/hearing_age.csv') %>% distinct(ChildStudyID, .keep_all = TRUE)
...
```{r, contending with autocorrelation for CIs, fig.show='hide',cache=T}
...
evt_model_CI <- ci_kids %>%
  bam(elog ~      Condition.ord + 
                  s(Time) +
                  s(Time, by=Condition.ord) +
                  te(Time,EVT_Age,by=Condition.ord) + 
                  te(Time,EVT_GSV,by=Condition.ord) + # ti requires us to specify individual smooths; te does not
                  s(Time, ChildStudyID, bs = "fs", m = 1) + 
                  s(Time, TargetWord, bs = "fs", m = 1) +
                  s(Time, num_visit, bs = "fs", m = 1) +
                  s(Time, ChildStudyID, by=Condition.ord, bs = "fs", m = 1), 
                  ...
                  AR.start=start.event)
gam.check(evt_model_CI) # looks good
...
AIC(evt_model_CI, gfta_model_CI) # gfta is lower

gfta_model_CI_freml <- ci_kids %>%
  bam(elog ~      Condition.ord + 
                  s(Time) +
                  s(Time, by=Condition.ord) +
                  te(Time,EVT_Age,by=Condition.ord) + 
...
# does gfta improve on an evt-only model?
gfta_evt_model_CI <- ci_kids %>%
  bam(elog ~      Condition.ord + 
                  s(Time) +
                  s(Time, by=Condition.ord) +
                  te(Time,EVT_Age,by=Condition.ord) + 
                  te(Time,GFTA_Standard,by=Condition.ord) + 
                  te(Time,EVT_GSV,by=Condition.ord) + 
                  s(Time, ChildStudyID, bs = "fs", m = 1) + 
                  s(Time, TargetWord, bs = "fs", m = 1) + 
                  s(Time, num_visit, bs = "fs", m = 1) +
                  s(Time, ChildStudyID, by=Condition.ord, bs = "fs", m = 1), 
                  ...
                  select=T, # impose penalties 
                  AR.start=start.event)
gam.check(gfta_evt_model_CI) 

AIC(gfta_model_CI,gfta_evt_model_CI) # more complex model is not justified; evt doesn't improve on gfta
compareML(gfta_model_CI,gfta_evt_model_CI) # more complex model is not justified
AIC(evt_model_CI,gfta_evt_model_CI) # gfta-evt model is justified; gfta improves on evt-only 
compareML(evt_model_CI,gfta_evt_model_CI) # gfta + evt minimal improvement 

saveRDS(gfta_model_CI_freml, "../all_child_models/ci/gfta-model-ci-freml.Rdata") 
saveRDS(gfta_evt_model_CI, "../all_child_models/ci/evt-gfta-model-ci.Rdata") # these were run with ML
saveRDS(gfta_model_CI, "../all_child_models/ci/gfta-model-ci.Rdata")
saveRDS(evt_model_CI, "../all_child_models/ci/evt-model-ci.Rdata")
```
...
gfta_model_CI_freml <- readRDS("../all_child_models/ci/gfta-model-ci-freml.Rdata")

# create tertiles
ci_kids3 <- ci_kids %>%
  mutate(tertile=as.factor(ntile(GFTA_Standard, 3))) %>%
  distinct(ChildStudyID, .keep_all = TRUE) %>%
  select(ChildStudyID,tertile,EVT_GSV,GFTA_Standard)
...
low <- get_smooths_difference(gfta_model_CI_freml, 
                       "Time", 
...
med <- get_smooths_difference(gfta_model_CI_freml, 
                       "Time", 
                       list(Condition.ord = c("real","MP")),
                       exclude_random = F,
...
get_smooths_difference(gfta_model_CI_freml, 
                       "Time", 
                       list(Condition.ord = c("real","MP")),
                       exclude_random = F,
...
```{r, read in all TH kids}
th_kids <- looks %>% filter(Group!='CochlearImplant')
```

```{r, contending with autocorrelation for THs, fig.show='hide',cache=T}
contrasts(th_kids$Condition.ord) <- "contr.treatment"
...
th_kids <- th_kids %>%
  ...
  mutate(WordGroup=as.factor(WordGroup)) %>%
  mutate(TargetWord=as.factor(TargetWord)) #%>%
  #filter(GFTA_Standard!='NA') # remove kids w/o gfta; all kids have EVT


# fit a model with the data properly ordered
m1_th <- bam(elog ~ Condition.ord +
                  s(Time) +
                  s(Time, by=Condition.ord) +
                  s(Time, ChildStudyID, bs = "fs", m = 1) +
                  s(Time, TargetWord, bs = "fs", m = 1) +
...
r1_th <- start_value_rho(m1_th) # first we calculate rho from the original model 
```

```{r, plotting autocorrelation for TH, fig.show='hide', eval=FALSE}
m_AR1_th <- bam(elog ~   Condition.ord +
                         s(Time) +
                         s(Time, by=Condition.ord) +
                         s(Time, ChildStudyID, bs = "fs", m = 1) +
                         s(Time, TargetWord, bs = "fs", m = 1) +
...
saveRDS(m_AR1_th, "../all_child_models/th/ARI-th.Rdata") 

# we can plot the improvement here:
m_acf_rmv_th <- acf_resid(m_AR1_th) # took care of the problem!
```

```{r, modeling TH ind differences, eval=FALSE}
# all 37 kids have evt scores
# 24/37 kids have gfta scores
# we only model the 24 kids who have both

# first check if vocab matters at all
age_model_th <- th_kids %>%
  bam(elog ~      Condition.ord + 
                  s(Time) +
                  s(Time, by=Condition.ord) +
                  te(Time,EVT_Age,by=Condition.ord) + 
...
evt_model_th <- th_kids %>%
  bam(elog ~      Condition.ord + 
                  ...
                  select=T, 
                  AR.start=start.event)
gam.check(evt_model_th) # looks good
compareML(age_model_th,evt_model_th) # evt doesn't improve upon baseline

gfta_model_th <- th_kids %>%
  bam(elog ~      Condition.ord + 
                  s(Time,k=15) +
                  ...
                  select=T, # impose penalties 
                  AR.start=start.event)
gam.check(gfta_model_th) # looks good
AIC(evt_model_th, gfta_model_th) # gfta is lower
AIC(age_model_th,gfta_model_th) # but higher than age model


saveRDS(age_model_th, "../all_child_models/th/age-model-th.Rdata")
saveRDS(gfta_model_th, "../all_child_models/th/gfta-model-th.Rdata")
saveRDS(evt_model_th, "../all_child_models/th/evt-model-th.Rdata")
```
...
all_cis <- read.csv('../participant_info/audiological_info.csv') %>% 
  distinct(ResearchID, .keep_all = TRUE) 
